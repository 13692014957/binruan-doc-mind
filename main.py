...
def _batch_text_to_embeddings(texts: list[str]) -> list[Optional[bytes]]:
    """
    批量将文本列表转为向量列表，大幅提升速度。
    返回的列表长度与输入相同，空文本对应 None。
    """
    if not texts:
        return []
    ...
    # 批量编码（一次调用，大幅提升速度）
    vecs = model.encode(valid_texts, convert_to_numpy=True, normalize_embeddings=True)
    ...
    for i, idx in enumerate(valid_indices):
        if i < len(vecs):
            buf = io.BytesIO()
            np.save(buf, vecs[i].astype(np.float32))
...
app = FastAPI(
    title="个人知识文档问答系统 API（含 AI 语义关联）",
...
# =====================
# 文档导入：txt / docx / md / pdf → 大模型解析 → 写入知识库
# 设计约束：
#   - txt  ≤ 1 MB，docx ≤ 2 MB，md ≤ 1 MB，pdf ≤ 2 MB（文件大小硬上限）
...
def _read_docx(raw: bytes) -> str:
    ...
    if len(text.strip()) < 50:
        print(f"[DEBUG] docx解析: 段落数={paragraph_count}, 表格数={table_count}, 总字数={len(text)}")
        if parts:
            print(f"[DEBUG] 前3段内容: {parts[:3]}")

    return text


def _read_markdown(raw: bytes) -> str:
    """读取 markdown 文件，保留结构化信息（标题、列表、代码块等），转换为纯文本。"""
    try:
    ...
    text = re.sub(r'___([^_]+)___', r'\1', text)        # ___加粗斜体___
    ...
    # 移除行首尾的管道符，并将管道符替换为空格
    text = re.sub(r'^[\s]*\|', '', text, flags=re.MULTILINE)
    text = re.sub(r'\|[\s]*$', '', text, flags=re.MULTILINE)
    text = text.replace('|', ' ')  # 将剩余的管道符替换为空格
...
def _read_pdf(raw: bytes) -> str:
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail="服务器未安装 pypdf，请先 pip install pypdf")
    ...
    # 添加调试信息
    print(f"[DEBUG] PDF解析: 总页数={total_pages}, 提取段数={len(parts)}, 总字数={len(text)}")
    if parts and len(text.strip()) < 100:
        print(f"[DEBUG] 前3段内容: {parts[:3]}")

    # 如果提取的文本过少，尝试其他方法
    if len(text.strip()) < 50 and total_pages > 0:
        # 可能PDF是扫描件或使用了特殊编码
        # 尝试逐页提取并保留更多格式信息
        parts_v2: list[str] = []
        for i, page in enumerate(reader.pages):
            text = page.extract_text()
            if text and text.strip():
                parts_v2.append(f"[第{i+1}页]\n{text.strip()}")

        if parts_v2:
            text = '\n\n'.join(parts_v2)
            text = text.replace("\r\n", "\n").replace("\r", "\n")
            print(f"[DEBUG] PDF二次提取: 段数={len(parts_v2)}, 总字数={len(text)}")
...
def _build_extract_prompt(chunk_text: str, chunk_index: int, total_chunks: int, per_chunk_max: int) -> tuple[str, str]:
    ...
    system = (
        ...
        "4) 即使文本很短，也要尽力提取其中的关键信息作为节点；只有完全无意义的乱码或空白才返回空数组 []；\n"
...
@app.post("/api/import/upload", response_model=ImportStatusResponse,
          summary="上传 txt/docx/md/pdf 并启动大模型解析")
async def import_upload(file: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    if lower.endswith(".txt"):
        ...
        file_type = "md"
        max_bytes = MAX_MD_BYTES
    elif lower.endswith(".pdf"):
        file_type = "pdf"
        max_bytes = MAX_PDF_BYTES
    else:
        raise HTTPException(status_code=400, detail="仅支持 .txt、.docx、.md 与 .pdf 文件。")
...
def import_confirm(import_id: str, body: ImportConfirmRequest, db: Session = Depends(get_db)):
    ...
    # 应用 overrides，收集所有需要生成向量的文本
    overrides = body.overrides or {}
    nodes_data: list[dict] = []
    texts_for_embedding: list[str] = []
    texts_indices: list[int] = []  # 记录每个文本对应的 nodes_data 索引
    ...
    if texts_for_embedding:
        try:
            embeddings = _batch_text_to_embeddings(texts_for_embedding)
            for i, idx in enumerate(texts_indices):
                if i < len(embeddings) and embeddings[i] is not None:
                    nodes_data[idx]["embedding"] = embeddings[i]
        except Exception as e:
            print(f"[WARN] 批量向量生成失败，将跳过所有向量：{e}")
    ...
    for data in nodes_data:
        try:
            node = KnowledgeNode(title=data["title"], content=data["content"],
                                 category=data["category"], tags=data["tags_str"],
                                 embedding=data["embedding"])
            db.add(node)
            db.flush()
            inserted_ids.append(int(node.id))
        except Exception as e:
            warnings.append(f"候选 #{data['cid']} 写入失败：{e}")
